...
class UserService {
    ...
    func registerUser(email: String, username: String, password: String, completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "http://159.54.129.113/users/register") else {
            completion(false)
            return
        }
    ...
    func loginUser(login: String, password: String, completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "http://159.54.129.113/users/login") else {
            completion(false)
            return
        }
    ...
    func addQuizPuntuation(userId: Int, score: Int, date: String, completion: @escaping (Bool) -> Void) {
            guard let url = URL(string: "http://159.54.129.113/users/quiz") else {
                completion(false)
                return
            }
    ...
    func getLastScores(userId: Int, completion: @escaping (Bool, [[String: Any]]?) -> Void) {
        let urlString = "http://159.54.129.113/users/lastscores/\(userId)"
    ...
    func changeUserPassword(userId: String, newPassword: String, completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "http://159.54.129.113/users/changepw/\(userId)") else {
            completion(false)
            return
        }
    ...
    func getUserInfo(userId: String, completion: @escaping (Bool, [String: Any]?) -> Void) {
        guard let url = URL(string: "http://159.54.129.113/users/userdata/\(userId)") else {
            completion(false, nil)
            return
        }
    ...
    func getUserQuizzesInfo(userId: String, completion: @escaping (Bool, [String: Any]?) -> Void) {
        guard let url = URL(string: "http://159.54.129.113/users/userquizzes/\(userId)") else {
            completion(false, nil)
            return
        }
    ...
    func getUserID(username: String) async throws {
        guard let url = URL(string: "http://159.54.129.113/users/getid/\(username)") else {
            print("URL inválida")
            return
        }
        print("PASO 1")
